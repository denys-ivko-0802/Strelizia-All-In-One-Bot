...
class StickyMessageListener (commands .Cog ):
    ...
    async def on_message (self ,message ):
        """Handle messages and update sticky messages"""
        ...
        enabled ,delay_seconds ,auto_delete_after ,ignore_bots ,
        ignore_commands ,trigger_count ,current_count )=sticky_data 
            ...
            if new_count >=trigger_count :
                ...
                if new_sticky_msg :
                    ...
                    if auto_delete_after >0 :
                        asyncio .create_task (self .auto_delete_message (new_sticky_msg ,auto_delete_after ))
            else :
    ...
    async def get_prefix (self ,message ):
        """Get the bot prefix for the guild"""
        try :
    ...
    async def update_counter (self ,guild_id ,channel_id ,new_count ):
        """Update the message counter for a sticky message"""
        async with aiosqlite .connect ("db/stickymessages.db")as db :
            await db .execute ("""
                UPDATE sticky_messages 
                SET current_count = ?
    ...
    async def send_sticky_message (self ,channel ,msg_type ,msg_content ,embed_data ):
        """Send a sticky message to the channel"""
        try :
    ...
    async def on_guild_channel_delete (self ,channel ):
        """Clean up sticky messages when a channel is deleted"""
        try :
            async with aiosqlite .connect ("db/stickymessages.db")as db :
                await db .execute (
    ...
    async def on_guild_remove (self ,guild ):
        """Clean up sticky messages when bot leaves a guild"""
        try :
            async with aiosqlite .connect ("db/stickymessages.db")as db :
                await db .execute (
